...
from tickManage import settings
from tickManage.lib import safe_template_context, process_attachments
from tickManage.models import Queue, Ticket, TicketCC, FollowUp, IgnoreEmail
...
def process_email(quiet=False):
    for q in Queue.objects.filter(
        ...
        logger = logging.getLogger('django.tickManage.queue.' + q.slug)
        logging_types = {
            'info': logging.INFO,
            'warn': logging.WARN,
            'error': logging.ERROR,
        ...
        logdir = q.logging_dir or '/var/log/tickManage/'
...
def process_queue(q, logger):
    logger.info("***** %s: Begin processing mail for django-tickManage" % ctime())
    ...
    if email_box_type in mail_defaults:
        ...
        mail_dir = q.email_box_local_dir or '/var/lib/mail/tickManage/'
...
def create_ticket_cc(ticket, cc_list):
    ...
    from tickManage.views.staff import User, subscribe_to_ticket_updates
    ...
    for part in message.walk():
        ...
        if part.get_content_maintype() == 'text' and name is None:
            if part.get_content_subtype() == 'plain':
                ...
                # https://github.com/django-tickManage/django-tickManage/issues/732
