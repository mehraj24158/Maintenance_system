...
from tickManage.decorators import tickManage_staff_member_required, protect_view
from tickManage import settings as tickManage_settings
from tickManage.views import feeds, staff, public, kb, login
try:
    import tickManage.tasks
except ImportError:
    pass
...
urlpatterns += [
    url(r'^rss/user/(?P<user_name>[^/]+)/$',
        tickManage_staff_member_required(feeds.OpenTicketsByUser()),
        name='rss_user'),
    ...
    url(r'^rss/queue/(?P<queue_slug>[A-Za-z0-9_-]+)/$',
        tickManage_staff_member_required(feeds.OpenTicketsByQueue()),
        name='rss_queue'),

    url(r'^rss/unassigned/$',
        tickManage_staff_member_required(feeds.UnassignedTickets()),
        name='rss_unassigned'),

    url(r'^rss/recent_activity/$',
        tickManage_staff_member_required(feeds.RecentFollowUps()),
        name='rss_activity'),
]
...
urlpatterns += [
    ...
    url(r'^logout/$',
        auth_views.LogoutView.as_view(
            template_name='tickManage/registration/login.html',
            next_page='../'),
        name='logout'),

    url(r'^password_change/$',
        auth_views.PasswordChangeView.as_view(
            template_name='tickManage/registration/change_password.html',
            success_url='./done'),
        name='password_change'),

    url(r'^password_change/done$',
        auth_views.PasswordChangeDoneView.as_view(
            template_name='tickManage/registration/change_password_done.html',),
        name='password_change_done'),
]

if tickManage_settings.TICKMANAGE_KB_ENABLED:
    urlpatterns += [
        url(r'^kb/$',
            kb.index,
            name='kb_index'),
...
urlpatterns += [
    url(r'^help/context/$',
        TemplateView.as_view(template_name='tickManage/help_context.html'),
        name='help_context'),

    url(r'^system_settings/$',
        login_required(DirectTemplateView.as_view(template_name='tickManage/system_settings.html')),
