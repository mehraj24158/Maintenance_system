...
class RequestUser:
    ...
    def get_queues(self):
        ...
        limit_queues_by_user = \
            tickManage_settings.TICKMANAGE_ENABLE_PER_QUEUE_STAFF_PERMISSION \
    ...
    def can_access_queue(self, queue):
        ...
        :param user: The User (the class should have the has_perm method)
        :param queue: The django-tickManage Queue instance
        :return: True if the user has permission (either by default or explicitly), false otherwise
        """
        if self.has_full_access():
            return True
        else:
            return tickManage_settings.TICKMANAGE_ENABLE_PER_QUEUE_STAFF_PERMISSION and self.user.has_perm(queue.permission_name)
