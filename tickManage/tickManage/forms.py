...
from tickManage.lib import safe_template_context, process_attachments
from tickManage.models import (Ticket, Queue, FollowUp, IgnoreEmail, TicketCC,
...
class AbstractTicketForm(CustomFieldMixin, forms.Form):
    ...
    priority = forms.ChoiceField(
        ...
        required=True,
        initial=getattr(settings, 'TICKMANAGE_PUBLIC_TICKET_PRIORITY', '3'),
        label=_('Priority'),
    ...
    def _create_ticket(self):
        ...
        ticket = Ticket(
            ...
            priority=self.cleaned_data.get(
                'priority',
                getattr(settings, "TICKMANAGE_PUBLIC_TICKET_PRIORITY", "3")
            ),
            due_date=self.cleaned_data.get(
                'due_date',
                getattr(settings, "TICKMANAGE_PUBLIC_TICKET_DUE_DATE", None)
            ) or None,
            kbitem=kbitem,
        )
    ...
    def _send_messages(ticket, queue, followup, files, user=None):
        ...
        if ticket.assigned_to and ticket.assigned_to.usersettings_tickManage.email_on_ticket_assign:
...
class TicketForm(AbstractTicketForm):
    ...
    assigned_to = forms.ChoiceField(
        widget=(
            ...
            if not tickManage_settings.TICKMANAGE_CREATE_TICKET_HIDE_ASSIGNED_TO
...
class PublicTicketForm(AbstractTicketForm):
    ...
    def __init__(self, hidden_fields=(), readonly_fields=(), *args, **kwargs):
        ...
        field_hide_table = {
            'queue': 'TICKMANAGE_PUBLIC_TICKET_QUEUE',
            'priority': 'TICKMANAGE_PUBLIC_TICKET_PRIORITY',
            'due_date': 'TICKMANAGE_PUBLIC_TICKET_DUE_DATE',
        }
    ...
    def _get_queue(self):
        if getattr(settings, 'TICKMANAGE_PUBLIC_TICKET_QUEUE', None) is not None:
...
class TicketCCForm(forms.ModelForm):
    ''' Adds either an email address or tickManage user as a CC on a Ticket. Used for processing POST requests. '''
    ...
    def __init__(self, *args, **kwargs):
        ...
        if tickManage_settings.TICKMANAGE_STAFF_ONLY_TICKET_CC:
...
class TicketCCUserForm(forms.ModelForm):
    ''' Adds a tickManage user as a CC on a Ticket '''
