...
from tickManage.models import Ticket, FollowUp, Queue

User = get_user_model()


class OpenTicketsByUser(Feed):
    title_template = 'tickManage/rss/ticket_title.html'
    description_template = 'tickManage/rss/ticket_description.html'
    ...
    def title(self, obj):
        if obj['queue']:
            return _("Service Request: Open Tickets in queue %(queue)s for %(username)s") % {
                'queue': obj['queue'].title,
                'username': obj['user'].get_username(),
            }
        else:
            return _("Service Request: Open Tickets for %(username)s") % {
                'username': obj['user'].get_username(),
            }
...
class UnassignedTickets(Feed):
    ...
    title = _('Service Request: Unassigned Tickets')
    description = _('Unassigned Open and Reopened tickets')
    link = ''  # '%s?assigned_to=' % reverse('tickManage:list')
...
class RecentFollowUps(Feed):
    title_template = 'tickManage/rss/recent_activity_title.html'
    description_template = 'tickManage/rss/recent_activity_description.html'

    title = _('Service Request: Recent Followups')
    description = _('Recent FollowUps, such as e-mail replies, comments, attachments and resolutions')
    link = '/tickets/'  # reverse('tickManage:list')
...
class OpenTicketsByQueue(Feed):
    ...
    def title(self, obj):
        return _('Service Request: Open Tickets in queue %(queue)s') % {
            'queue': obj.title,
        }
