...
from tickManage.query import (
    get_query_class,
    query_to_base64,
    query_from_base64,
)

from tickManage.user import RequestUser

from tickManage.decorators import (
    tickManage_staff_member_required, tickManage_superuser_required,
    is_tickManage_staff
)
from tickManage.forms import (
    TicketForm, UserSettingsForm, EmailIgnoreForm, EditTicketForm, TicketCCForm,
    TicketCCEmailForm, TicketCCUserForm, EditFollowUpForm, TicketDependencyForm
)
from tickManage.decorators import superuser_required
from tickManage.lib import (
    safe_template_context,
    process_attachments,
    queue_template_context,
)
from tickManage.models import (
...
from tickManage.views.permissions import MustBeStaffMixin
...
if tickManage_settings.TICKMANAGE_ALLOW_NON_STAFF_TICKET_UPDATE:
...
@tickManage_staff_member_required
def dashboard(request):
    ...
    if request.user.is_authenticated and hasattr(request.user, 'usersettings_tickManage'):
        tickets_per_page = request.user.usersettings_tickManage.tickets_per_page
    else:
        tickets_per_page = 25
    ...
    huser = RequestUser(request.user)
    ...
    queues = RequestUser(request.user).get_queues().values_list('id', flat=True)

    from_clause = """FROM    tickManage_ticket t,
                    tickManage_queue q"""
    if queues:
    ...
    return render(request, 'tickManage/dashboard.html', {
...
def delete_ticket(request, ticket_id):
    ...
    if request.method == 'GET':
        return render(request, 'tickManage/delete_ticket.html', {
            'ticket': ticket,
        })
    else:
        ticket.delete()
...
def followup_edit(request, ticket_id, followup_id):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'tickManage/followup_edit.html', {
            'followup': followup,
            'ticket': ticket,
            'form': form,
        ...
        if form.is_valid():
            ...
            # delete old followup
            followup.delete()
        return HttpResponseRedirect(reverse('tickManage:view', args=[ticket.id]))
...
def view_ticket(request, ticket_id):
    ...
    if tickManage_settings.TICKMANAGE_STAFF_ONLY_TICKET_OWNERS:
        users = User.objects.filter(is_active=True, is_staff=True).order_by(User.USERNAME_FIELD)
    else:
        users = User.objects.filter(is_active=True).order_by(User.USERNAME_FIELD)

    queues = RequestUser(request.user).get_queues()
    ...
    return render(request, 'tickManage/ticket.html', {
        'ticket': ticket,
...
def update_ticket(request, ticket_id, public=False):
    ...
    if not (public or (
            ...
            request.user.is_active and (
                is_tickManage_staff(request.user) or
        ...
        if not ticket:
            return HttpResponseRedirect(
                '%s?next=%s' % (reverse('tickManage:login'), request.path)
            )
    ...
    if no_changes:
        return return_to_ticket(request.user, tickManage_settings, ticket)
    ...
    comment = comment.replace('{%', 'X-TICKMANAGE-COMMENT-VERBATIM').replace('%}', 'X-TICKMANAGE-COMMENT-ENDVERBATIM')
    comment = comment.replace('X-TICKMANAGE-COMMENT-VERBATIM', '{% verbatim %}{%').replace('X-TICKMANAGE-COMMENT-ENDVERBATIM', '%}{% endverbatim %}')
    ...
    if public and (f.comment or (
        ...
        if ticket.assigned_to and ticket.assigned_to.usersettings_tickManage.email_on_ticket_change:
    ...
    if ticket.assigned_to and (ticket.assigned_to.usersettings_tickManage.email_on_ticket_change or (reassigned and ticket.assigned_to.usersettings_tickManage.email_on_ticket_assigned)):
    ...
    if tickManage_settings.TICKMANAGE_AUTO_SUBSCRIBE_ON_TICKET_RESPONSE and request.user.is_authenticated:
...
def return_to_ticket(user, tickManage_settings, ticket):
    """Helper function for update_ticket"""

    if is_tickManage_staff(user):
...
def mass_update(request):
    ...
    for t in Ticket.objects.filter(id__in=tickets):
        ...
        if action == 'assign' and t.assigned_to != user:
            ...
            if t.assigned_to and t.assigned_to.usersettings_tickManage.email_on_ticket_change:
...
def ticket_list(request):
    ...
    return render(request, 'tickManage/ticket_list.html', dict(
        context,
        default_tickets_per_page=request.user.usersettings_tickManage.tickets_per_page,
...
def datatables_ticket_list(request, query):
    ...
    """
    query = Query(RequestUser(request.user), base64query=query)
...
def edit_ticket(request, ticket_id):
    ...
    return render(request, 'tickManage/edit_ticket.html', {'form': form, 'ticket': ticket})
...
class CreateTicketView(MustBeStaffMixin, abstract_views.AbstractCreateTicketMixin, FormView):
    template_name = 'tickManage/create_ticket.html'
    ...
    def get_form_kwargs(self):
        ...
        queues = RequestUser(self.request.user).get_queues()
    ...
    def get_success_url(self):
        ...
        if RequestUser(request.user).can_access_queue(self.ticket.queue):
            return self.ticket.get_absolute_url()
        else:
            return reverse('tickManage:dashboard')
...
def rss_list(request):
    return render(request, 'tickManage/rss_list.html', {'queues': Queue.objects.all()})
...
def report_index(request):
    ...
    user_queues = RequestUser(request.user).get_queues()
    ...
    return render(request, 'tickManage/report_index.html', {
...
def run_report(request, report):
    if Ticket.objects.all().count() == 0 or report not in (
        ...
        return HttpResponseRedirect(reverse("tickManage:report_index"))

    report_queryset = Ticket.objects.all().select_related().filter(
        queue__in=RequestUser(request.user).get_queues()
    )
    ...
    if report == 'userpriority':
        ...
        col1heading = _('User')
        queue_options = RequestUser(request.user).get_queues()
    ...
    return render(request, 'tickManage/report_output.html', {
        'title': title,
        'charttype': charttype,
        'data': table,
...
def save_query(request):
    ...
    return HttpResponseRedirect('%s?saved_query=%s' % (reverse('tickManage:list'), query.id))
...
def delete_saved_query(request, id):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'tickManage/confirm_delete_saved_query.html', {'query': query})
...
class EditUserSettingsView(MustBeStaffMixin, UpdateView):
    template_name = 'tickManage/user_settings.html'
    form_class = UserSettingsForm
    model = UserSettings
    success_url = reverse_lazy('tickManage:dashboard')

    def get_object(self):
        return UserSettings.objects.get_or_create(user=self.request.user)[0]


@tickManage_superuser_required
def email_ignore(request):
    return render(request, 'tickManage/email_ignore_list.html', {
...
def email_ignore_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse('tickManage:email_ignore'))
    else:
        form = EmailIgnoreForm(request.GET)

    return render(request, 'tickManage/email_ignore_add.html', {'form': form})
...
def email_ignore_del(request, id):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'tickManage/email_ignore_del.html', {'ignore': ignore})
...
def ticket_cc(request, ticket_id):
    ...
    return render(request, 'tickManage/ticket_cc_list.html', {
        'copies_to': copies_to,
        'ticket': ticket,
    })
...
def ticket_cc_add(request, ticket_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ticketcc.save()
            return HttpResponseRedirect(reverse('tickManage:ticket_cc',
                                                kwargs={'ticket_id': ticket.id}))
    else:
        form_email = TicketCCEmailForm()
        form_user = TicketCCUserForm()
    return render(request, 'tickManage/ticket_cc_add.html', {
        'ticket': ticket,
...
def ticket_cc_del(request, ticket_id, cc_id):
    ...
    return render(request, 'tickManage/ticket_cc_del.html', {'cc': cc})
...
def ticket_dependency_add(request, ticket_id):
    ...
    return render(request, 'tickManage/ticket_dependency_add.html', {
        'ticket': ticket,
        'form': form,
    })
...
def ticket_dependency_del(request, ticket_id, dependency_id):
    ...
    if request.method == 'POST':
        dependency.delete()
        return HttpResponseRedirect(reverse('tickManage:view', args=[ticket_id]))
    return render(request, 'tickManage/ticket_dependency_del.html', {'dependency': dependency})
...
def attachment_del(request, ticket_id, attachment_id):
    ...
    if request.method == 'POST':
        attachment.delete()
        return HttpResponseRedirect(reverse('tickManage:view', args=[ticket_id]))
    return render(request, 'tickManage/ticket_attachment_del.html', {
