...
logger = logging.getLogger('tickManage')
...
def send_templated_mail(template_name,
    ...
    from tickManage.models import EmailTemplate
    from tickManage.settings import TICKMANAGE_EMAIL_SUBJECT_TEMPLATE, \
        TICKMANAGE_EMAIL_FALLBACK_LOCALE

    locale = context['queue'].get('locale') or TICKMANAGE_EMAIL_FALLBACK_LOCALE
    ...
    subject_part = from_string(
        TICKMANAGE_EMAIL_SUBJECT_TEMPLATE % {
            "subject": t.subject
        }).render(context).replace('\n', '').replace('\r', '')

    footer_file = os.path.join('tickManage', locale, 'email_text_footer.txt')

    text_part = from_string(
        "%s{%% include '%s' %%}" % (t.plain_text, footer_file)
    ).render(context)

    email_html_base_file = os.path.join('tickManage', locale, 'email_html_base.html')
